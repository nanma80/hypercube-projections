#!/usr/bin/env python3
"""
Investigate n→3 projections for n = 15, 18.
Runs generic optimization (no structural assumptions), computes symmetry,
saves results to files to avoid data loss.

Usage: python investigate_n_3.py <n> [--trials N] [--niter N]
"""
...
def get_generators(P, n, k):
    """Get generators from tight-frame P as unit-norm vectors."""
    c = (P @ P.T)[0, 0]
...
def main():
    ...
    # Save generators before computing symmetry (in case symmetry is slow)
    outfile = f"results_{n}_3.txt"
    with open(outfile, 'w') as f:
        f.write(f"{n}->3 max-volume projection results\n")
        f.write(f"Volume: {vol:.15f}\n")
        f.write(f"Equilateral: {equil}\n")
        f.write(f"Norm std: {np.std(norms):.10f}\n\n")
        f.write(f"Generators ({n} vectors in R^3):\n")
        for i, (g, nm) in enumerate(zip(gens, norms)):
            f.write(f"  g{i:2d} = ({g[0]:+.10f}, {g[1]:+.10f}, {g[2]:+.10f})  |g|={nm:.8f}\n")
    ...
    print(f"Distinct inner products: {len(unique_ips)}")
    with open(outfile, 'a') as f:
    ...
    # Absolute value profiles (sorted |coords|)
    print(f"\nGenerator profiles (sorted |coords|):")
    profiles = {}
    for i, g in enumerate(gens):
        p = tuple(sorted(np.round(np.abs(g), 4)))
    ...
    for p in sorted(profiles.keys()):
        idxs = profiles[p]
        print(f"  {p}: {len(idxs)} generators (indices {idxs})")
        with open(outfile, 'a') as f:
